...
Optional config at repo root, .agentsync.json — all keys optional:

    {
      ...
      "codex":       true,      // .codex/agents/*.toml, and assert .codex/skills is absent
      "max_lines":   200,       // warn above this
      "hard_lines":  250        // --check fails above this
    }

Skills live once, in .agents/skills: Codex reads that folder itself, and Claude reads it
through the .claude/skills symlink. The openspec-* skills in it are generated by
`openspec update`, not by this script.
...
def check_no_codex_skills(root: Path, rep: Report, apply: bool) -> None:
    """Assert .codex/skills does not exist: Codex reads .agents/skills itself.

    Codex CLI 0.94+ loads repo skills from .agents/skills, and OpenSpec renders its Codex
    skills there as well, so a .codex/skills mirror only makes Codex list every skill twice.
    --apply removes the folder when it holds nothing but symlinks; real content inside it
    may be someone's work, so moving that into .agents/skills is left to a human.
    """
    codex = root / ".codex" / "skills"
    if not codex.exists() and not codex.is_symlink():
        rep.ok(".codex/skills absent — Codex reads .agents/skills")
        return

    rep.problem(".codex/skills exists; Codex reads .agents/skills directly, so this mirror lists every skill twice")
    if not apply:
        return
    if codex.is_symlink():
        codex.unlink()
        rep.action("removed the .codex/skills symlink")
    elif codex.is_dir() and all(entry.is_symlink() for entry in codex.iterdir()):
        shutil.rmtree(codex)
        rep.action("removed .codex/skills (it held only symlinks)")
    else:
        rep.problem(".codex/skills holds real content; move anything worth keeping to .agents/skills and delete it")
...
def sync_skills(root: Path, cfg: dict, rep: Report, apply: bool, migrate: bool) -> None:
    ...
    if cfg["codex"]:
        check_no_codex_skills(root, rep, apply)
