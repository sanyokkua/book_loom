...
Optional config at repo root, .agentsync.json — all keys optional:

    {
      ...
      "codex_skills_passthrough": []   // .codex/skills/<name> dirs that are real,
                                        // harness-specific vendored content (e.g. generated
                                        // with Codex-native invocation syntax) and must stay
                                        // real directories instead of becoming symlinks into
                                        // .agents/skills. Every other canonical skill name still
                                        // gets a per-entry symlink into .codex/skills.
    }
...
def sync_codex_skills(root: Path, cfg: dict, rep: Report, apply: bool) -> None:
    """Per-entry mirror into .codex/skills, skipping names in codex_skills_passthrough.

    Those names are real, harness-specific vendored content (e.g. OpenSpec's own
    Codex-flavoured skill text, which uses `$name` invocation instead of Claude's
    `/opsx:` slash commands) — a whole-directory symlink would silently overwrite them
    with the Claude-authored copy. Every other canonical skill still gets a plain
    per-entry symlink so Codex can reach it.
    """
    canonical = root / ".agents" / "skills"
    codex = root / ".codex" / "skills"
    passthrough = set(cfg.get("codex_skills_passthrough", []))
    ...
    if codex.is_symlink():
        if passthrough:
            rep.problem(
                ".codex/skills is a whole-directory symlink, but codex_skills_passthrough "
                f"names real vendored dirs {sorted(passthrough)} that a directory symlink "
                "would hide. Remove the symlink; per-entry sync will recreate it."
            )
            return
    ...
    if codex.exists() and not codex.is_dir():
        rep.problem(".codex/skills exists and is not a directory")
        return
    ...
    if not codex.is_dir():
        if apply:
            ...
            rep.problem(".codex/skills is missing (should hold passthrough dirs + per-skill symlinks)")
            return

    for name in sorted(passthrough):
        entry = codex / name
        if entry.is_dir() and not entry.is_symlink():
            rep.ok(f".codex/skills/{name} kept as vendored (passthrough)")
        else:
            rep.problem(
                f".codex/skills/{name} should be a real vendored directory (passthrough); "
                f"found {'a symlink' if entry.is_symlink() else 'nothing'}"
            )

    canonical_names = {p.name for p in canonical.iterdir() if p.is_dir()}
    for name in sorted(canonical_names - passthrough):
        sync_symlink(root, f".codex/skills/{name}", f"../../.agents/skills/{name}", rep, apply)

    for entry in sorted(codex.iterdir()):
        if entry.name in passthrough or entry.name in canonical_names:
            continue
        if entry.is_symlink():
            rep.problem(f".codex/skills/{entry.name} is an orphaned symlink (no canonical source)")
            if apply:
                entry.unlink()
                rep.action(f"removed orphaned .codex/skills/{entry.name}")
...
def sync_skills(root: Path, cfg: dict, rep: Report, apply: bool, migrate: bool) -> None:
    ...
    if cfg["codex"]:
        sync_codex_skills(root, cfg, rep, apply)
