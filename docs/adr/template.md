# ADR-NNNN — <decision title, one imperative phrase>

**Status:** proposed | accepted | superseded by ADR-MMMM | deprecated **Date:** YYYY-MM-DD
**Deciders:** <roles or names>
**Supersedes:** ADR-MMMM (omit when none)

## Context and problem statement

<What forces the decision. Link the DD-NN and the problem.>

## Decision drivers

- <driver>

## Considered options

- <option A>
- <option B>

## Decision outcome

Chosen: <option>, because <reason>.

### Consequences

- Positive: <…>
- Negative: <…>
- Neutral: <…>

## Pros and cons of the options

### Option A — <name>

- Good: <…>
- Bad: <…>

## Links

- Design decisions: <DD-NN>
- Spec clauses: <in-repo docs/specification/... files>
- Stories: <STORY-NNN, or "none yet">
