...
| 5. Translate | Translating   | Start the run; the pipeline processes the whole book automatically; the user may pause, resume, stop (a terminal `stopped` state; a new run starts from the beginning), or leave.                                                                                                                                                                                                         | FR-ALGO-01, FR-RESUME-03, FR-RESUME-05 |
| 6. Review    | Review        | Optionally review the flagged queue side by side and accept/edit/retry/skip.                                                                                                                                                                                                                                                                                 | FR-REVIEW-01..07                       |
| 7. Export    | Export        | The run has already written the book (the save path is chosen on the Book Brief); this screen reports the written file and its format — fixed to the original, export is same-format-only (DD-30) — and shows it in the file manager. The final consistency pass and side exports are later.                                                                                                                                            | FR-EXPORT-01..06                       |
...
| User pause                  | Pause aborts the request in flight and the run halts at once; the interrupted segment is redone on resume; progress is checkpointed; resume continues from the first PENDING segment.                                                                  |
| User stop                   | Stop aborts the request in flight and ends the run in the terminal `stopped` state (FR-RESUME-05). Cancellation is never surfaced as an error; nothing is written, and only a new run, from the beginning, is offered — resuming a stopped run from the first PENDING segment arrives with local storage.                   |
